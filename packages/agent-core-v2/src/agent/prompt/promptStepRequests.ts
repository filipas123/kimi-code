 ...
 * `appendPrompt` ordering. `SteerStepRequest` is mergeable (folds into the
 * next step's driver) and survives turn boundaries (drained by a later run);
 * it records the `turn.steer` wire op on materialization and unregisters
 * itself from the service's pending-steer set once settled.
 * `RetryStepRequest` contributes no message and simply drives one more step
 * over the existing context. Constructed by the prompt service with its
 * collaborators captured — these are plain runtime objects, not DI services.
 */

import type { ContextMessage } from '#/agent/contextMemory/types';
import { StepRequest, type StepRequestOptions } from '#/agent/loop/stepRequest';
