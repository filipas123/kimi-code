 ...
 * auto-refresh cadence). Registered into `IConfigRegistry` by
 * `ModelCatalogService` on construction, mirroring the per-domain
 * `configSection.ts` convention (see `background/configSection.ts`), so the
