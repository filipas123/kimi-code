...
- [ ] 排查 `activateAgentServices()` 懒代理“假预热”：审计已完成，裸 `accessor.get(...)` 对 delayed service 不能作为完整预热屏障。已补 `IPermissionRulesService.rules` 强制构造以恢复 session approval replay；已补 `ISwarmMode.isActive` 强制构造以恢复 `AgentSwarm` service-owned tool 注册；已补 `IProfileService.data()`、`IPermissionModeService.mode`、`IPermissionService.data()` 以激活 config / permission restore handlers 和初始 permission mode；已补 `IBackgroundService.list` 强制构造以恢复 background restore/reconcile hooks；已补 `IContextUsageService.getStatus()` 强制构造以恢复 usage restore/context usage hooks；已补 `IAgentSkillService.activate` 强制构造以恢复 `skill.activate` replay handler，覆盖 `skill-tool-manager.test.ts`。restore 预热已补 `IToolStoreService.data()`、`IUserToolService.register` 和 `IGoalService.getGoal()`。后续优先核对/修复构造期有 wire handler、hook、tool、event listener 的高风险服务：`IPlanModeService`、`IMcpRuntimeService`、`IFullCompaction`、`IMicroCompactionService`、`ILoopService`。覆盖测试按对应服务单文件收口，不做一次性大改。
